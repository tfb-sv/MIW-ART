...
class Classifier(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.args = args
        if self.args.act_func == "ReLU": self.actv_func = nn.ReLU()
        elif self.args.act_func == "Tanh": self.actv_func = nn.Tanh()
...
class ARTM_model(nn.Module):
    def __init__(self, args):
        ...
        if args.task == "clf": self.sigmoid = nn.Sigmoid()
        self.reset_parameters()
    ...
    def forward(self, encoded_ligand, ligand_length):
        ...
        x = self.classifier(h)
        if self.args.task == "clf": x = self.sigmoid(x)
        elif self.args.task == "reg": pass
